...
exports.cleanupCategories = async () => {
  try {
    ...
    //intialize a counter to tract the number of deleted categories
    let deletedCount = 0;
    ...
    for (const item of Items) {
      ...
      const deleteCommand = new DeleteItemCommand({
        TableName: tableName,
        Key: { fileName: item.fileName }, //Delete using  the primary key
      });
    ...
    // Send an SNS notificatoin after deleting categories
    const snsMessage = `Cleaned up. Deleted ${deletedCount} outdated categories`;
